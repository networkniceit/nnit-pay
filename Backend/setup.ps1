# NNIT Pay - Full Setup Script for PowerShell
# Run this file: .\setup.ps1

$base = "C:\Users\netwo\nnit-pay"
$backend = "$base\backend"

Write-Host "Creating NNIT Pay project at $base..." -ForegroundColor Cyan
...
router.post("/register", async (req, res) => {
  try {
    ...
    const user = { id: uuidv4(), name, email, phone: phone || "", country: country || "DE", password: hashedPassword, kycStatus: "pending", createdAt: new Date() };
    db.users.push(user);
    db.wallets.push({ id: uuidv4(), userId: user.id, balance: 0, currency: "EUR" });
...
router.get("/me", (req, res) => {
  ...
  try {
    const { userId } = jwt.verify(token, process.env.JWT_SECRET || "nnitpay_secret");
...
router.post("/", authenticate, (req, res) => {
  try {
    ...
    if (!receiverEmail || !amount || !currency) return res.status(400).json({ error: "receiverEmail, amount and currency required" });
    ...
    if (sender.id === receiver.id) return res.status(400).json({ error: "Cannot send to yourself" });
    ...
    if (senderWallet.balance < totalDeducted) return res.status(400).json({ error: `Insufficient funds. Need EUR ${totalDeducted} (incl. EUR ${fee} fee), have EUR ${senderWallet.balance}` });
    ...
    const tx = { id: uuidv4(), senderId: sender.id, senderName: sender.name, senderEmail: sender.email, receiverId: receiver.id, receiverName: receiver.name, receiverEmail: receiver.email, amount: parseFloat(amount), currency, amountInEUR, fee, totalDeducted, note: note || "", status: "completed", type: "transfer", createdAt: new Date() };
...
router.post("/quote", (req, res) => {
  try {
    ...
    if (!amount || !fromCurrency || !toCurrency) return res.status(400).json({ error: "amount, fromCurrency, toCurrency required" });
    ...
    res.json({ send: parsedAmount, fromCurrency, fee, amountAfterFee, toCurrency, receiverGets: converted, exchangeRate: rate, estimatedTime: "1-3 minutes" });
...
router.get("/", authenticate, (req, res) => {
  const txs = db.transactions
    ...
    .map(t => ({ ...t, direction: t.senderId === req.userId ? "sent" : "received" }));
  res.json(txs);
});

router.get("/:id", authenticate, (req, res) => {
  const tx = db.transactions.find(t => t.id === req.params.id && (t.senderId === req.userId || t.receiverId === req.userId));
...
app.get("/", (req, res) => res.json({
  name: "NNIT Pay API", version: "2.0.0", status: "running",
  endpoints: {
...
app.listen(PORT, () => {
  console.log("\n NNIT Pay API running on http://localhost:" + PORT);
